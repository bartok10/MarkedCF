...
for p in range(len(mark)):
	...
	mean_m_GR2 = np.mean(np.loadtxt(dir_2+'marks/GR_mark_p'+mark[p]+'_'+box+'.txt'))
	DD_GR = np.loadtxt(dir_1+'pairs/D1D2_JK64_GR_'+box+'.txt')
	mm_GR=np.loadtxt(dir_1+'pairs/m1m2_JK64_GR_mark_p'+mark[p]+'_'+box+'.txt')
	...
	print 'save file:' + dir_1+'data/m_mark'+mark[p]+'_GR_logsm0p3_1p7_B'+box+'.txt\n'
	np.savetxt(dir_1+'data/m_mark'+mark[p]+'_GR_logsm0p3_1p7_B'+box+'.txt',S.T)
	print '...done.'
	V_C = np.zeros((N,N))
...
#	np.savetxt(dir_out+'covM_GR_mark'+mark[p]+'_norm.txt',V_C)
	...
	for q in range(len(MG)):
		...
		mean_m_MG = np.mean(m)
		mean_m_MG2 = np.mean(np.loadtxt(dir_2+'marks/'+MG[q]+'_mark_p'+mark[p]+'_'+box+'.txt'))
		DD_MG = np.loadtxt(dir_1+'pairs/D1D2_JK64_'+MG[q]+'_'+box+'.txt')
	        mm_MG=np.loadtxt(dir_1+'pairs/m1m2_JK64_'+MG[q]+'_mark_p'+mark[p]+'_'+box+'.txt')
        	dMG = mm_MG/(DD_MG*mean_m_MG*mean_m_MG2)
		MG_mean = np.mean(dMG,axis=0)
		S = MG_mean
		print 'save file:' + dir_1+'data/m_mark'+mark[p]+'_'+MG[q]+'_logsm0p3_1p7_B'+box+'.txt'
		np.savetxt(dir_1+'data/m_mark'+mark[p]+'_'+MG[q]+'_logsm0p3_1p7_B'+box+'.txt',S.T)
		print '...done.'
