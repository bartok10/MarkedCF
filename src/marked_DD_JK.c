...
//	char archivo[128];
//	strcpy(archivo,"/gpfs/data/jarmijo/"); // ojo que el folder es fijo
//	strcat(archivo,input);
...
//    char archivo[64];
//	strcpy(archivo,"/gpfs/data/jarmijo/"); // fix folder
//	strcat(archivo,filename);
	printf("file: %s ...\n ",filename);
...
double ** mm_2p(double * X,double * Y,double * Z,double * V,long int N,int Nl,float Lbox,int bins,double LIMIT,float dmin,float dmax,int JK){
	int JK_index,c,d,n;
...
int main(int argc, char **argv){
	char PATH[64];	strcpy(PATH,argv[4]);
...
//----------------------------------------------------------//
	...
	strcpy(px,PATH); strcpy(py,PATH); strcpy(pz,PATH);strcpy(pv,PATH);
	...
	strcat(px,name_x); strcat(py,name_y); strcat(pz,name_z); strcat(pv,name_v);
	...
	col_v = malloc(sizeof(double)*length);
        ...
        col_v = load_file(pv,length);

        printf("calculating...\n");
     	xi = mm_2p(col_x,col_y,col_z,col_v,length,iN,lbox,Bins,limit,smin,smax,jk); //correr funcion
	printf("done!");
       	char NAME[32];
        snprintf(NAME, sizeof(char)*32, "mm_JK64_%s_mark_p%s_%s.txt",Mod,mark,lab); //archivo final. Cada linea es un JK. Tiene 2 mitades: 1 para los pares, otra para pares marcados
       	char OUT[256];
        strcpy(OUT,PATH); // directory for the output file
	strcat(OUT,"pairs/");
       	strcat(OUT,NAME);
